...
describe("OrderForm (create)", () => {
  it("renders, picks unit and product, computes total, submits without unitPrice", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Margherita G/i)).toBeInTheDocument(),
    );
...
describe("OrderForm (edit)", () => {
  ...
  it("pre-populates fields from initial", async () => {
    ...
    await waitFor(() =>
      expect(
        (screen.getByLabelText(/observações/i) as HTMLInputElement).value,
      ).toBe("sem cebola"),
    );
    const itemsBlock = screen.getByTestId("ord-items");
    const rows = within(itemsBlock).getAllByTestId("ord-item-row");
    ...
    expect(
      (within(rows[0]).getByLabelText(/quantidade/i) as HTMLInputElement).value,
    ).toBe("2");
  });
  ...
  it("submits as PUT and redirects to detail on edit", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Centro", { selector: '[data-slot="select-value"]' }),
      ).toBeInTheDocument(),
    );
