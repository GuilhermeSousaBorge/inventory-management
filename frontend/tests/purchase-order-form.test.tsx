...
describe("PurchaseOrderForm (create)", () => {
  it("renders, adds an item, prefills unitPrice, computes total, submits", async () => {
    ...
    await waitFor(() =>
      expect(
        (within(firstRow).getByLabelText(/preço/i) as HTMLInputElement).value,
      ).toBe("23.5"),
    );
    ...
    await waitFor(() =>
      expect(screen.getByTestId("po-total").textContent).toMatch(/47/),
    );
    ...
    await waitFor(() => {
      const post = getCalls().find(
        (c) => c.method === "post" && c.url?.endsWith("/purchase-orders"),
      );
