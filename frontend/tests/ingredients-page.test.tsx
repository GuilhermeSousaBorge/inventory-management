...
describe("IngredientsPage", () => {
  it("renders rows with resolved category and supplier names", async () => {
    ...
    setHandler((cfg) => {
      ...
      if (url.endsWith("/categories"))
        return categoriesPage([{ id: CAT_ID, name: "Massas" }]);
      if (url.endsWith("/suppliers"))
        return suppliersPage([{ id: SUP_ID, name: "Distribuidora ABC" }]);
  ...
  it("EMPLOYEE does not see the create button", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/nenhum ingrediente cadastrado/i),
      ).toBeInTheDocument(),
    );
    expect(
      screen.queryByRole("link", { name: /novo ingrediente/i }),
  ...
  it("respects ?category and ?active in URL", async () => {
    ...
    expect(ingredientsCall?.params).toMatchObject({
      category: CAT_ID,
      active: false,
    });
  });
