...
describe("UsersPage", () => {
    it("EMPLOYEE sees NoAccess", async () => {
        ...
        await waitFor(() => expect(screen.getByText(/sem permissão/i)).toBeInTheDocument())
    ...
    it("OWNER with empty list sees empty state and create CTA", async () => {
        ...
        renderWithProviders(<UsersPage />)
        await waitFor(() => expect(screen.getByText(/nenhum usuário cadastrado/i)).toBeInTheDocument())
        expect(screen.getByRole("button", { name: /criar primeiro usuário/i })).toBeInTheDocument()
    })
        ...
        setHandler((cfg) => {
            ...
            if (url.includes("/users") && cfg.method === "get") {
                return {
                    ...
                    data: {
                        data: [
                            { id: "u1", name: "Ana Owner", email: "ana@x.com", role: "OWNER", active: true, createdAt: "2026-01-01T00:00:00Z" },
                            { id: "u2", name: "Bruno", email: "bruno@x.com", role: "EMPLOYEE", active: false, createdAt: "2026-02-01T00:00:00Z" },
                        ],
                        page: 0,
                        size: 20,
                        total: 2,
