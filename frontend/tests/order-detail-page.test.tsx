...
describe("OrderDetailPage", () => {
    it("PENDING + OWNER shows Editar/Iniciar/Cancelar", async () => {
        ...
        await waitFor(() =>
            expect(screen.getByRole("heading", { name: /Pedido #11111111/i })).toBeInTheDocument(),
        )
    ...
    it("clicking Iniciar opens dialog and triggers POST /start on confirm", async () => {
        ...
        await waitFor(() =>
            expect(screen.getByText(/serão descontados do estoque/i)).toBeInTheDocument(),
        )

        // The dialog has its own Iniciar button — click the last "Iniciar" button (in the dialog)
        const iniciarButtons = screen.getAllByRole("button", { name: /^iniciar$/i })
