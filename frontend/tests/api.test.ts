...
describe("api response interceptor — envelope unwrap", () => {
    it("unwraps single-resource { data } envelope", async () => {
        handler = () => ({ status: 200, data: { data: { id: "u1", name: "Ana" } } })
...
describe("api error normalization", () => {
    it("400 with errors array → ApiError with fieldErrors", async () => {
        handler = () => ({ status: 400, data: { errors: ["nome obrigatório", "email inválido"] } })
...
describe("api refresh-on-401", () => {
    it("refreshes once for parallel 401s (single-flight) and retries originals", async () => {
        ...
        const responses = new Map<string, MockResponse[]>([
            ["/units", [{ status: 401 }, { status: 200, data: { data: [], page: 0, size: 20, total: 0 } }]],
            ["/users/me", [{ status: 401 }, { status: 200, data: { data: { id: "u1" } } }]],
            [
                "/auth/refresh",
                [{ status: 200, data: { data: { accessToken: "new-access", refreshToken: "new-refresh" } } }],
            ],
        ])
        ...
        const [unitsRes, meRes] = await Promise.all([api.get("/units"), api.get("/users/me")])
