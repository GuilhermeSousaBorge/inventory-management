...
describe("PurchaseOrderDetailPage", () => {
  it("OWNER + PENDING shows Receive and Cancel; clicking Receive triggers POST", async () => {
    ...
    setHandler((cfg) => {
      ...
      if (
        url.includes("/purchase-orders/po1/receive") &&
        cfg.method === "post"
      ) {
    ...
    // confirm dialog
    await waitFor(() =>
      expect(screen.getByText(/recebimento/i)).toBeInTheDocument(),
    );
    // click confirm in dialog (label = "Receber" inside dialog)
    const dialogConfirm = screen
      .getAllByRole("button", { name: /receber/i })
      .pop()!;
  ...
  it("RECEIVED PO shows no actions", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /^receber$/i }),
