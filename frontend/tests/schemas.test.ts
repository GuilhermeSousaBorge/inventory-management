...
describe("createAdjustmentSchema", () => {
  ...
  it("rejects non-positive quantity", () => {
    expect(
      createAdjustmentSchema.safeParse({ ...validBase, quantity: 0 }).success,
    ).toBe(false);
    expect(
      createAdjustmentSchema.safeParse({ ...validBase, quantity: -1 }).success,
    ).toBe(false);
  });
  ...
  it("rejects non-uuid ingredientId/unitId", () => {
    expect(
      createAdjustmentSchema.safeParse({ ...validBase, ingredientId: "no" })
        .success,
    ).toBe(false);
    expect(
      createAdjustmentSchema.safeParse({ ...validBase, unitId: "no" }).success,
    ).toBe(false);
  });
});
...
describe("createPurchaseOrderSchema", () => {
  ...
  it("rejects non-uuid supplierId/unitId", () => {
    expect(
      createPurchaseOrderSchema.safeParse({ ...validBase, supplierId: "no" })
        .success,
    ).toBe(false);
    expect(
      createPurchaseOrderSchema.safeParse({ ...validBase, unitId: "no" })
        .success,
    ).toBe(false);
  });
});
...
describe("createProductSchema", () => {
  ...
  it("rejects duplicated ingredient ids", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues.some((i) => /duplicad/i.test(i.message))).toBe(
        true,
      );
    }
  });
