...
describe("ProductsPage", () => {
  it("OWNER on active list sees Edit and Deactivate actions", async () => {
    ...
    setHandler((cfg) => {
      ...
      if (url.includes("/products") && cfg.method === "get")
        return listHandler(true);
      return { status: 500 };
    });
    ...
    expect(
      screen.getByLabelText(/^desativar margherita g$/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("EMPLOYEE sees no mutating actions", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: /novo produto/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText(/^editar margherita g$/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText(/^desativar margherita g$/i),
    ).not.toBeInTheDocument();
  });
